...
export const AnalysisResults = ({ data, imageUrl, onRequestRecipe }) => {
  ...
  const uniqueIngredients = data.detected_ingredients || [
    ...new Set(data.allPredictions?.map((p) => p.label) || []),
  ];

  // Main food name: join first 3 unique ingredients
  const mainFoodDisplay = uniqueIngredients.slice(0, 3).join(", ");
  const hasMoreIngredients = uniqueIngredients.length > 3;
      ...
      <div className="card p-6 space-y-4">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-4">
            ...
            />
            <div>
              {/* UPDATED: Show multiple ingredients */}
              <h2 className="text-2xl font-bold text-gray-800">
                {mainFoodDisplay}
                {hasMoreIngredients && (
                  <span className="text-lg text-gray-500 ml-2">
                    +{uniqueIngredients.length - 3} lainnya
                  </span>
                )}
              </h2>
              <div className="flex items-center gap-3 mt-1">
                <span className="text-sm text-gray-500">
                  {uniqueIngredients.length} Bahan Terdeteksi
                </span>
        ...
        {/* Detected Ingredients Summary - NEW SECTION */}
        {uniqueIngredients.length > 0 && (
          <div className="bg-blue-50 rounded-lg p-4 border border-blue-200">
            <h3 className="text-sm font-semibold text-blue-900 mb-2 flex items-center gap-2">
              🥘 Bahan Makanan Terdeteksi ({uniqueIngredients.length}):
            </h3>
            <div className="flex flex-wrap gap-2">
              {uniqueIngredients.map((ingredient, idx) => (
                <span
                  key={idx}
                  className="bg-white text-blue-700 px-3 py-1.5 rounded-full text-sm font-medium border border-blue-300 shadow-sm"
                >
                  {ingredient}
                </span>
              ))}
        ...
        {data.allPredictions && data.allPredictions.length > 1 && (
          <div className="bg-gray-50 rounded-lg p-4">
            <h3 className="text-sm font-semibold text-gray-700 mb-2">
              📊 Detail Deteksi ({data.allPredictions.length}):
            </h3>
              ...
              {data.allPredictions.map((pred, idx) => (
                ...
                >
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-sm text-emerald-600 font-semibold w-12 text-right">
                      {pred.confidence}%
                    </span>
                  </div>
                </div>
      ...
      {data.recommended_recipes && data.recommended_recipes.length > 0 ? (
        <div className="bg-white rounded-lg shadow-md p-6">
          <h2 className="text-2xl font-bold text-gray-800 mb-4 flex items-center gap-2">
            🍳 Rekomendasi Resep ({data.recommended_recipes.length})
          </h2>
          <p className="text-sm text-gray-600 mb-4">
            Berdasarkan {uniqueIngredients.length} bahan yang terdeteksi
          </p>
        ...
        <div className="bg-yellow-50 rounded-lg shadow-md p-6 border border-yellow-200">
          <h2 className="text-xl font-bold text-yellow-800 mb-2 flex items-center gap-2">
            ⚠️ Tidak Ada Rekomendasi Resep
          </h2>
          <p className="text-sm text-yellow-700">
            Maaf, belum ada resep yang cocok dengan kombinasi bahan:{" "}
            <strong>{uniqueIngredients.join(", ")}</strong>
          </p>
        </div>
      )}
    </div>
